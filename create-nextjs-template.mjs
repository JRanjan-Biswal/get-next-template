...
const git_repo_nextjs_strapi_template = "https://github.com/JRanjan-Biswal/strapi-nextjs-template.git";
...
try {
        ...
        choices: [
            {
                ...
                value: 'snst',
                description: 'A simple and clean template to start with nextjs and strapi',
            },
            {
                name: 'nextjs template',
                value: 'snt',
                description: 'A simple and clean template to start with nextjs'
            }
        ]
    })
    ...
    else if (template === 'snst') {
        // clone the repo into the project folder -> creates the new boilerplate
        const gitSpinner = ora("Downloading files...").start();
        await exec(`git clone --depth 1 ${git_repo_nextjs_strapi_template} ${projectPath} --quiet`);
        gitSpinner.succeed();

        // remove my git history + remove the installation file
        const cleanSpinner = ora("Removing useless files").start();
        const rmGit = rm(path.join(projectPath, ".git"), { recursive: true, force: true });
        const rmBin = rm(path.join(projectPath, "bin"), { recursive: true, force: true });
        await Promise.all([rmGit, rmBin]);
